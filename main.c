...
#define MAX_SCRIPT_SIZE 65536 // 2730 InvisiBytes
...
int main(int argc, char *argv[]){
    ...
    // printf("First three bytes: %02x %02x %02x\n", firstThreeBytes[0], firstThreeBytes[1], firstThreeBytes[2]);
    unsigned char lastByte;
    fseek(scriptFile, -1, SEEK_END);
    fread(&lastByte, 1, 1, scriptFile);
    // printf("Last byte: %02x\n", lastByte);
    
    // char sourceCode[] = [MAX_SCRIPT_SIZE];
